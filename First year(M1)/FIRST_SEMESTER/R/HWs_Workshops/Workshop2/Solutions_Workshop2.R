...
ggplot(data = iris) + geom_histogram(x = aes(Sepal.Length))
...
ggplot(data = diamonds,
  ...
  geom_point(mapping = aes(col = cut), size=0.7) +
  geom_smooth(method = 'lm') +
  facet_wrap(~color) 
...
# Modify the color to using the command : scale_fill_manual.
mycol=c("yellow", "green", "pink")
ggplot(sw) + geom_bar(aes(x = gender, fill = gender)) + labs(title = "Gender distribution of the sw Universe",  
                                                                  x = "Gender", y = "Frequency")+ 
  scale_fill_manual(name = "Gender:", values = mycol, na.value = mycol[3])

#Draw the densities for the height of feminines and masculines 
...
#Draw a segmented barplot for the variable 'sex'. The colors depend on the hair colors 
