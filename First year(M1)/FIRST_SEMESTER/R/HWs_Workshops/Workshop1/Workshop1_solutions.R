### Lesson 1 / 2 Processing larbe dataset with R
### Workshop 1 
### October 19th 2021
#####WORKSHOP 1####
...
##    with n = 1000 and shape parameter respectively: 1, 2 , 3, 4. 
...
## - the first column represents the age, distribuited as a random uniform,from 20 to 40 years old, 
...
celsius = fahrenheit_to_celsius(19)


## 1c)
set.seed(123)
t = sample(v, 100, replace = T, prob=runif(length(v)))
t
which.max(table(t)) #To check
hist(t, freq = F, breaks = length(v), col = "yellow", main = "Sampling with replacement")

#1d)

A=matrix(rnorm(56, 1,2), nrow = 7, ncol = 8, byrow=TRUE)
...
##2a)
a= filter(flights, month == 4 & day == 8)%>%
...
#2c
Lax= flights %>% 
  filter(dest == "LAX", na.rm = TRUE) %>% 
  na.omit() %>%
  select(dep_delay, arr_delay) %>% 
  arrange(desc(dep_delay), na.rm = TRUE)
